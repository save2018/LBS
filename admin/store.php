...
if ($rec == 'default') {
     ...
     */
    if ($keyword) {
        $where =   " where name LIKE '%$keyword%' or phone like '%$keyword%' or store_id like '%$keyword%'";
        $get = '&keyword=' . $keyword;
    }
    ...
    $page_url = 'store.php' . ($cat_id ? '?cat_id=' . $cat_id : '');
    ...
    while ($row = $dou->fetch_array($query)) {
        ...
        $store_list[] =  array (
              ...
              //  "add_time" => $add_time,
        		...
        		"district"=>$district,
        		"status"=>$row['status'],
        		'baidu_coordinate'=>$row['baidu_coordinate']
        );
    }
    //print_r($store_list);
